...
| **放射照度スペクトル** | 波長ごとの光の強さ。各ランプ単独点灯時の `light_*.csv` がこれです。 |
...
1. **材料をそろえる（Step 1）**  
   各サブセルの SR、3 つのランプのスペクトル、AM0 表、基準セルの測定値などを CSV から読みます。＝レシピの材料確認。

2. **混ぜ合わせ比率を決める（Step 2, Eq.(2)）**  
   「top 用の光エネルギー」「mid 用」「bot 用」が目標に近づくように、ランプ A・B・C の明るさ係数 **A_i** を連立方程式で求めます。＝ソースの配合表 `step02_*.csv` が出力されます。
...
- **Q. なぜ「3 ランプ × 3 サブセル」など数がそろうの？**  
  **A.** Eq.(2) は「サブセルごとの目標積分」を「光源の積分の線形結合」で当てはめる **連立方程式**なので、係数行列が正方になるように **光源本数 = サブセル数** という前提になっています（実装は [`solve_eq2`](mj_solar_uncertainty/core.py)）。
...
- `step02_A.csv`, `step02_M.csv`, `step02_b.csv` - Eq.(2) の係数と右辺
